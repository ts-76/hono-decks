...
| PDF export | Print-to-PDF compatible output | smoke | `examples/basic` includes `smoke:pdf`, which starts `wrangler dev`, saves sample/motion print pages to PDF, and checks A4 handout page counts. Visual PDF inspection remains a release checklist item. |
