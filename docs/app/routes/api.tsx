...
const runtimeDefinitions: ApiDefinition[] = [
  { id: "define-decks", symbol: "defineDecks", entry: "hono-decks", signature: "defineDecks(options: DecksOptions): DefinedDecks", description: { ja: "生成済みのマニフェストから、デッキの取得元とルーターを作成します。", en: "Creates a source and router factory from a generated manifest." }, when: { ja: "コンパイラーが生成するモジュールの仕組みを拡張するとき。通常のアプリでは直接呼び出しません。", en: "Extending generated module behavior. Ordinary applications do not call it directly." }, guide: "/docs/getting-started", source: "server/define-decks.ts" },
  { id: "decks-router", symbol: "decksRouter", entry: "hono-decks", signature: "decksRouter(options: DecksRouterOptions): Hono", description: { ja: "DeckSourceを使って、一覧、ビューアー、発表者画面などのルートを作成します。", en: "Mounts a DeckSource across index, viewer, presenter, and other route surfaces." }, when: { ja: "独自のDeckSourceからルーターを組み立てるとき。生成済みデッキではcreateDecksRouter()を使う方が簡単です。", en: "Building a router from a custom DeckSource. Generated decks are simpler through createDecksRouter()." }, guide: "/docs/routing", source: "server/router.ts" },
  { id: "deck-context", symbol: "deckContext", entry: "hono-decks", signature: "deckContext(options: DeckContextOptions): MiddlewareHandler", description: { ja: "アプリ側のルートから、コンパイル済みデッキ、ビューアーの部品、目次、メタデータを参照できるようにします。", en: "Adds the compiled deck, viewer parts, TOC, and metadata to app-owned routes." }, when: { ja: "独自の詳細画面、管理画面、OG画像用ルートをHonoアプリ側で作るとき。", en: "Creating app-owned detail, admin, or social-image routes for a deck." }, guide: "/docs/routing", source: "server/router.ts" },
  { id: "define-decks-config", symbol: "defineDecksConfig", entry: "hono-decks", signature: "defineDecksConfig(config: DecksConfig): DecksConfig", description: { ja: "アプリ側の設定を、型を保ったまま定義します。", en: "Defines app-owned configuration while preserving its types." }, when: { ja: "環境変数、R2、CSP、公開画面などの設定をdecks.config.tsへまとめるとき。", en: "Collecting bindings, R2, CSP, or public-surface settings in decks.config.ts." }, guide: "/docs/configuration", source: "server/define-decks.ts" },
  { id: "merge-router-options", symbol: "mergeDecksRouterOptions", entry: "hono-decks", signature: "mergeDecksRouterOptions(base, overrides): DecksRouterOptions", description: { ja: "基本設定と上書き設定を結合します。入れ子になった設定も保持されます。", en: "Merges base and application/request overrides across nested options." }, when: { ja: "共通設定を保ったまま、呼び出し元でviewerやdocumentの一部だけを上書きするとき。", en: "Applying call-site viewer or document overrides without losing nested base settings." }, guide: "/docs/configuration", source: "server/define-decks.ts" },
];

const renderingDefinitions: ApiDefinition[] = [
  { id: "create-viewer-parts", symbol: "createDeckViewerParts", entry: "hono-decks", signature: "createDeckViewerParts(input: { deck; mountPath; controls?; exportPaths? }): Promise<DeckViewerParts>", description: { ja: "iframe、操作ボタン、目次、ページ情報を個別に取得します。", en: "Returns frame, controls, TOC, and page metadata as independent parts." }, when: { ja: "標準ビューアーを使わず、部品をアプリ独自のレイアウトへ配置するとき。", en: "Composing viewer parts into an application-owned layout instead of using the default viewer." }, guide: "/docs/routing", source: "server/viewer.ts" },
...
export default createRoute((c) => {
  ...
  return c.render(
    <DocsLayout activePath="/api" title={title} description={description} sections={sectionsByLocale[locale]} locale={locale}>
      <aside class="version-note"><strong>hono-decks 0.1.0</strong><span>{isJa ? "アプリが直接選ぶ主要APIを、package.jsonのexportsと実装に基づいて説明します。" : "This page explains the primary APIs applications choose directly, based on package exports and source."}</span></aside>
      <section id="start">
        <h2>{isJa ? "通常は生成されたcreateDecksRouter()から始める" : "Most applications start with generated createDecksRouter()"}</h2>
