...
export interface DeckMiddlewareOptions {
...
export function deckMiddleware(options: DeckMiddlewareOptions = {}): MiddlewareHandler {
  return async (c, next) => {
    ...
    return c.html(renderDeckPage({
      title: options.title ?? deck.slides[0]?.title ?? "Hono Decks",
      slideHtml,
...
async function resolveMarkdown(c: Context, options: DeckMiddlewareOptions): Promise<string> {
...
export function renderDeckPage(input: {
  ...
  const warnings = input.warnings?.length
    ? `<aside class="hono-decks-warnings">${input.warnings.map((warning) => `<p>${escapeHtml(warning)}</p>`).join("")}</aside>`
    : "";
...
<body>
  ${warnings}
  <main class="hono-decks-deck">${input.slideHtml}</main>
</body>
</html>`;
}
