...
export function hydrateSlideIslands(input: HydrateSlideIslandsInput): void {
  ...
  const islands = root.querySelectorAll<HTMLElement>("[data-hono-slides-island]");

  for (const island of islands) {
    const name = island.dataset.honoSlidesIsland;
    if (!name) continue;
    const component = input.components[name];
    if (!component) continue;

    const props: Record<string, SlidePropValue> = parseIslandProps(island.dataset.honoSlidesProps);
