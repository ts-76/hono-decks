...
`hono-decks.config.ts`がcompileとmount pathの共通設定です。`src/decks.ts`は編集可能なfacade、`src/generated`以下は自動生成です。`bun run dev`を起動するとWrangler custom buildが初回compileとMDX変更の監視を行います。
