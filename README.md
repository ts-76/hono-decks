...
生成される `src/generated/decks.ts` は Worker bundle に deck module を含めるための generated entry です。app の主 entry では直接 import せず、`src/decks.ts` のような user-owned facade に閉じ込めると、開発中の route code と generated code の距離を保てます。custom page や `deckContext()` も同じ facade から export した `deckSource` を共有できます。
